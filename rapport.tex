...
\begin{titlepage}
  ...
  \begin{center}
    ...
    % Title
    \HRule \\[0.4cm]
    { \huge \bfseries HUFFMAN}{\bfseries  \\[0.4cm] }
...
En premier temps la réalisation de cette lecture serait faite de manière préfixe, avec une variable binaire qui contiendrait le codage du caractère, s'incrémentant par un décalage de bit à chaque mouvement dans l'arbre jusqu'à attendre une feuille. 
...
(A voir) La décompression peut se faire de deux manières : l'en-tête peut contenir le dictionnaire et donc on lira le fichier compressé et on retrouvera le caractère correspondant au codage. Cette méthode est plus simple à réaliser pour l'en-tête, mais un peu plus complexe pour la décompression.
...
huffman est intéressant sur de gros fichiers mais pour un petit fichier le fichier originale est plus petit que celui compressé.
On a fait un tableau récapitulatif de différente compression pour comparer a l'algorithme d'Huffman.
...
\newline
\newline
En comparant ces différents algorithme one remarque que l'algorithme d'Huffman a un bon taux de compression 
avec un taux de 43\%. Bzip2 est celui avec le meilleur taux de compression de 55\%.
\newline
On voit donc que l'algorithme d'huffman seul est assez efficace comparé aux autres format de compression.


\section{Amélioration}

Notre code n'étant pas fini, de nombreux point peut être amélioré.
...
Ce projet nous a permis de nous améliorer dans le langage c, de voir comment gérer les caractères accentués. 
Ils nous a aussi permis de revoir les listes , les arbres, la lecture et l'écriture de fichier. 
On a ainsi pu voir la mise en place des bitwize en c malgré que cela ne soit pas implémenté dans notre code.
